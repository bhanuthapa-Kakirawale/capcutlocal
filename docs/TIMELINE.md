...
Each op checks preconditions and builds the result with Immer. Failures are values: `EditError` has a `code` (`VALIDATION`, `NOT_FOUND`, `TRACK_NOT_EMPTY`, `TRACK_LOCKED`, `COLLISION`) and a message. Locked tracks reject all ops on their contents (`TRACK_LOCKED`). Bounds that a real drag can legitimately overshoot — media handles, a neighboring clip, the maximum trim on either side of a roll edit — are never errors: the op silently clamps the requested delta to the nearest valid value and always succeeds (a delta of exactly zero is a no-op, returning the same `Project` reference). There's no separate "clamp amount" in the result; the caller diffs the returned project against the one it passed in to see what was actually applied, which every op already returns in full.
...
| `trimClip` | Move the head (changes `start`, `sourceIn`, `duration`) or tail (changes `duration`), clamped by media bounds and (unless `ripple`) the neighboring clip. `ripple: true` shifts later clips (and their linked partners) instead of leaving or overlapping a gap. | P4 |
| `splitClips` | Razor at *t* (frame-aligned) on the given clips, or every clip under *t* on unlocked tracks if none are given. Produces a new right-hand clip per split; right halves of clips linked to each other in the same call share a new linkId. A no-op if nothing spans *t*. | P4 |
...
- Ops locate a clip by scanning the **plain pre-image** `Project` passed in (before calling `produce`), not the Immer draft: reading through a draft's proxied array costs roughly 2µs per element regardless of what's read, so a full scan of a 5,000-clip track costs ~10 ms through a draft versus a few hundred microseconds against plain data (measured directly, `src/domain/ops/performance.test.ts`). The result — an index, or a `TrackId → number[]` map for a ripple — is then used to touch the draft only at those exact positions. There is no persistent `clipId → { trackId, index }` cache kept across calls; each op recomputes what it needs from the project it's given, which is fast enough at this scale without one.
- A consequence of Immer: any edit that changes a track's clip **count** (insert, split, lift, ripple-delete) costs time proportional to how many elements sit between the edit point and the end of that track's array, the same as a native array splice — cheap near the end of a long timeline, more expensive near the front. An edit that only changes existing clips' fields (trim, roll, the ripple-shift inside closeGap/rippleDelete/ripple-trim) costs time proportional only to how many clips actually move, independent of the track's total length. `performance.test.ts` documents this with one stress case (rippling an entire 5,000-clip track from the front) that intentionally does not meet the 2 ms budget below, alongside the realistic local-edit cases that do.
...
- **Budget:** every op < 2 ms on a local edit within a 5,000-clip sequence (`src/domain/ops/performance.test.ts`, using repeated warmed-up calls — a single cold call costs several extra milliseconds of JIT warm-up unrelated to the op itself).
- **Deferred:** the timeline view does not yet virtualize (every clip in view renders a DOM node regardless of how many tracks/clips exist) — not a problem at the scale P4 exercises by hand, but a real gap before a several-thousand-clip project would stay smooth to scroll. A future phase that needs it would window clips by visible time range the same way the architecture doc anticipated.
